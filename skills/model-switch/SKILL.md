---
name: model-switch
description: 当用户想要切换 AI 提供商或模型时使用。支持多个提供商，每个提供商有多个模型。切换作用域：项目级（默认）或全局。修改后需要重启 Claude Code 客户端生效。
---
...
7. 告知用户: "切换完成。请重启 Claude Code 客户端使新配置生效（可使用 `/resume` 恢复当前会话）。"
